...
bool timesBlinked[3] = {false, false, false};
int blinkCounter = 0;
bool touched = false;
...
static void blink_leds() {
  ...
  for (i = 0; i < counterLeds; i++){
    leds_values[i] = true;
  }
  /*if(counter < counterLeds) {
    counter++;
  }
...
static void keepLedsOn() {
  int i;
  for (i = 0; i < counterKeepOn; i++) {
    leds_values[i] = true;
  }
  /*static int counter = 0;
  counter++;
...
static bool get_sensor_input() {
  ...
  for (i = 0; i < DISTANCE_SENSORS_NUMBER; i++) {
    ...
    //printf("%f", distance_sensors_values[i]);
...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    keepLedsOn();
    if ((get_sensor_input() && (wb_motor_get_velocity(left_motor) > 2.2 && wb_motor_get_velocity(left_motor) < 2.35 || wb_motor_get_velocity(left_motor) > 2.40) && (wb_motor_get_velocity(right_motor) > 2.2 && wb_motor_get_velocity(right_motor) < 2.5 || wb_motor_get_velocity(right_motor) > 2.65 && wb_motor_get_velocity(right_motor) < 2.8  || wb_motor_get_velocity(right_motor) > 2.88)  && counterLeds < 9)){
      touched = true;
    } else {
      isTouching = false;
    }
    if(/*isTouching == true*/ touched){
      if(counterLeds < 9 && timesBlinked[2] == false){
        counterLeds++;
        isTouching = true;
      } else if(!isTouching && timesBlinked[2] == true){
        timesBlinked[0] = false;
        timesBlinked[1] = false;
        timesBlinked[2] = false;
        touched = false;
        if(counterKeepOn <= 9)
          counterKeepOn++;
        blinkCounter = 0;
    ...
    if(counterLeds >= 9){
      timesBlinked[blinkCounter] = true;
      blinkCounter++;
      resetLeds();
    }
    if (cliff_detected()) {
